...
* 微队列执行完后再执行宏队列
* 微任务在 DOM 渲染前触发， 宏任务在 DOM 渲染后触发（为什么微任务要比宏任务快，微任务是 ES6 语法规定的，宏任务是浏览器规定的）
* DOM 渲染则是在 call Stack 清空（即每轮循环结束，也就是异步代码执行结束）或者同步任务结束后都会去尝试触发 DOM 渲染，如果 DOM 渲染那么就会刷新页面
